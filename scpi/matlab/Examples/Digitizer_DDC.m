function Digitizer_DDC()
% Digitizer_DDC - MATLAB版本数字转换器DDC数据采集
% 功能：配置数字转换器、DDC处理、数据采集、数据保存和分析
% 作者：基于Python版本转换
% 日期：2024

fprintf('=== 数字转换器DDC数据采集 (MATLAB版本) ===\n');
fprintf('所有依赖库已成功导入！\n\n');
...
fprintf('配置DDC...\n');
try
    UltraBVC.send_command(sprintf(':DAQ:ACQuire:ExtractMultiple %s,%d', module_name, extract_multiple));
    ...
    for i = 1:length(channel_keys)
        ...
        UltraBVC.send_command(sprintf(':DAQ:CHANnel:DDC:EN %s,%s', key, logical_to_string(value)));
        if value
            UltraBVC.send_command(sprintf(':DAQ:CHANnel:DDC:NCOFrequence %s,%.6f', key, NCO_freq));
        end
    end
...
try
    ...
    % end
    % 
    % file_name = fullfile(save_file_path, sorted_list{1}, 'CH1_IQ_0.data'); % 原始数据路径
