function Digitizer_Stream()
% Digitizer_Stream - MATLAB版本数字转换器流模式数据采集
% 功能：配置数字转换器、流模式、数据采集、数据保存和分析
% 作者：基于Python版本转换
% 日期：2024

fprintf('=== 数字转换器流模式数据采集 (MATLAB版本) ===\n');
fprintf('所有依赖库已成功导入！\n\n');
...
try
    ...
    if strcmp(mode, 'RingBuffer')
        fprintf('切换到流模式...\n');
        UltraBVC.send_command(sprintf(':DAQ:ACQuire:WKMode %s,Stream', module_name));
        fprintf('已切换到流模式\n');
