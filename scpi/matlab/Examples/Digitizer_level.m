function Digitizer_level()
% Digitizer_level - MATLAB版本数字转换器电平触发数据采集
% 功能：配置数字转换器、电平触发、数据采集、数据保存和分析
% 作者：基于Python版本转换
% 日期：2024

fprintf('=== 数字转换器电平触发数据采集 (MATLAB版本) ===\n');
fprintf('所有依赖库已成功导入！\n\n');
...
try
    % 触发源为电平触发
    UltraBVC.send_command(sprintf(':DAQ:TRIGger:SOURce %s,Level', module_name));
    
    % 幅度触发电平
    channel_keys = keys(channel_en);
    for i = 1:length(channel_keys)
        key = channel_keys{i};
        UltraBVC.send_command(sprintf(':DAQ:TRIGger:EDGe:LEVel %s,%.6f', key, level_value));
    end
