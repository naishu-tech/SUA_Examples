...
try
    ...
    for i = 1:length(bin_files)
        ...
        file_len = file_info.bytes;
        
        % 添加波形列表
        UltraBVC.send_command(sprintf(':AWG:WAVList:ADDList %s,seg%d,%d,Int16', module_name, i-1, file_len/2));
        ...
        try
            ...
            wave_data = fread(fileID, file_len/2, 'int16');
            fclose(fileID);
            
            % 发送波形数据
            UltraBVC.send_command(sprintf(':AWG:WAVLIST:WAVeform:RTTRACE %s,seg%d,0,%d', module_name, i-1, file_len/2));
            
            % 这里需要实现实际的数据上传，可能需要使用HTTP请求
            % 由于MATLAB的限制，这里只是示例代码
            fprintf('波形数据上传完成: %s\n', bin_files(i).name);
        ...
        seg_list{end+1} = sprintf('seg%d', i-1);
    end
...
try
    ...
    % 构建NSQC代码
    nsqc = "@nw.kernel\n";
    nsqc = [nsqc, "def program(wlist: dict[str, np.ndarray]):\n"];
    
    for i = 1:length(seg_list)
        nsqc = [nsqc, sprintf("    seg%d: nw.ArbWave = nw.init_arbwave(wlist, '%s')\n", i-1, seg_list{i})];
    end
    
    nsqc = [nsqc, "    while True:\n"];
    
    for i = 1:length(seg_list)
        nsqc = [nsqc, sprintf("        nw.play_arb(seg%d)  # Play segment\n", i-1)];
    end
    
    nsqc = [nsqc, "    return nw.Kernel()\n"];
...
function success = upload_nswave_http(device_ip, nswave_name, nswave_program)
    ...
    try
        ...
        try
            ...
            % 添加文件数据
            wave_data = char(nswave_program(:)'); % 确保是行向量
            post_data = [post_data, wave_data];
