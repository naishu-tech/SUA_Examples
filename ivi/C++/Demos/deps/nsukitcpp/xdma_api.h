...
#define API_VERSION  20241129

//v20241129 : 1. FPGA to FPGA direct with infinite mode.
//            2. GPU direct support (must work with driver 20241120)
//v20241013 : 1. GPU direct support (must work with driver 20241013)
//            2. FPGA to FPGA direct support. 
//               new function fpga_direct_addr() and fpga_direct_size()
//            3. Add addr and length alignment checking in build_descriptors(). 
//linux driver 20241013 cuda p2p support
//v20240906 : 1. ring buffer accept no-4KB unit size. new fpga_ring_setup().
//v20231213 : 1. change the speed calculation base from 1024 to 1000
//v20231209 : 1. change MAX_NUM_CARDS to 32
//linux driver 20231206 undef the __LIBXDMA_DEBUG__ based on 20221219
...
DLLEXPORT unsigned int _API_CALL  fpga_info_vendor_id(unsigned int boardn);                        // Device ID                              
DLLEXPORT unsigned int _API_CALL  fpga_info_device_id(unsigned int boardn);                        // Vendor ID                              
DLLEXPORT unsigned int _API_CALL  fpga_info_bus_num(unsigned int boardn);                          // Bus number                             
DLLEXPORT unsigned int _API_CALL  fpga_info_device_num(unsigned int boardn);                       // Device number                          
DLLEXPORT unsigned int _API_CALL  fpga_info_function_num(unsigned int boardn);                     // Function number                        
DLLEXPORT unsigned int _API_CALL  fpga_info_max_lane_width(unsigned int boardn);                   // Max lane width, 1 2 4 8 16 32          
DLLEXPORT unsigned int _API_CALL  fpga_info_max_link_speed(unsigned int boardn);                   // Max link speed, 1=2.5G, 2=5G, 3=8G     
DLLEXPORT unsigned int _API_CALL  fpga_info_lane_width(unsigned int boardn);                       // Working lane width, 1 2 4 8 16 32      
DLLEXPORT unsigned int _API_CALL  fpga_info_link_speed(unsigned int boardn);                       // Working link speed, 1=2.5G, 2=5G, 3=8G 
DLLEXPORT unsigned int _API_CALL  fpga_info_max_payload_size(unsigned int boardn);                 // Max payload size in bytes              
DLLEXPORT unsigned int _API_CALL  fpga_info_max_readreq_size(unsigned int boardn);                 // Max read request size in bytes         
DLLEXPORT unsigned int _API_CALL  fpga_info_msi_enable(unsigned int boardn);                       // MSI Interrupt enabled                  
DLLEXPORT unsigned int _API_CALL  fpga_info_msix_enable(unsigned int boardn);                      // MSIX Interrupt enabled                 
DLLEXPORT unsigned int _API_CALL  fpga_info_msi_num(unsigned int boardn);                          // Number of MSIX Interrupt vectors       
DLLEXPORT unsigned int _API_CALL  fpga_info_axis_width(unsigned int boardn);                       // Bit width of AXIS interface            
DLLEXPORT unsigned int _API_CALL  fpga_info_relaxed_ordering(unsigned int boardn);                 // Relaxed ordering                       
DLLEXPORT unsigned int _API_CALL  fpga_info_ip_version(unsigned int boardn);                       // IP core version                        
DLLEXPORT unsigned int _API_CALL  fpga_info_kernel_version(unsigned int boardn);                   // OS kernel driver version               
DLLEXPORT unsigned int _API_CALL  fpga_info_api_version(unsigned int boardn);                      // API version                            
DLLEXPORT unsigned int _API_CALL  fpga_info_bar_num(unsigned int boardn);                          // Number of BARs                         
DLLEXPORT unsigned int _API_CALL  fpga_info_bar_reg(unsigned int boardn);                          // BAR index for XDMA control registers   
DLLEXPORT unsigned int _API_CALL  fpga_info_bar_alite(unsigned int boardn);                        // BAR index for AXI Lite                 
DLLEXPORT unsigned int _API_CALL  fpga_info_bar_bypass(unsigned int boardn);                       // BAR index for DMA Bypass               
DLLEXPORT unsigned int _API_CALL  fpga_info_chnl_num(unsigned int boardn, int c2h);                // Dma channels count (0-H2C, 1-C2H)      
DLLEXPORT unsigned int _API_CALL  fpga_info_stream(unsigned int boardn, int c2h, int chnl);        // If the dma channel is AXI-Stream

//for FPGA to FPGA P2P
DLLEXPORT unsigned int _API_CALL  fpga_direct_addr(unsigned int boardn);                           // BAR address for DMA Bypass               
DLLEXPORT unsigned int _API_CALL  fpga_direct_size(unsigned int boardn);                           // BAR length for DMA Bypass   
//Write a 32bit word to a XDMA bypass space
DLLEXPORT void _API_CALL fpga_write_direct(unsigned int boardn, unsigned int addr8, unsigned int data);
//Read a 32bit word from a XDMA bypass space
DLLEXPORT unsigned int _API_CALL fpga_read_direct(unsigned int boardn, unsigned int addr8);            
...
DLLEXPORT HANDLE _API_CALL fpga_alloc_dma(unsigned int boardn, unsigned long long len32, void* buffer = NULL,
                                HANDLE share_dma = NULL);
...
DLLEXPORT HANDLE _API_CALL fpga_alloc_dma_send(unsigned int boardn, unsigned long long len32, void* buffer = NULL, 
                                HANDLE share_dma = NULL);
//For P2P dma with GPU
DLLEXPORT HANDLE _API_CALL fpga_alloc_dma_cuda(unsigned int boardn, unsigned long long len32, void* buffer);
//For P2P dma with a general PCIe device
DLLEXPORT HANDLE _API_CALL fpga_alloc_dma_direct(unsigned int boardn, unsigned long long len32, unsigned long long bar_addr);
...
//#define DMA_OFFSET_INFINITE (~0ULL)

DLLEXPORT unsigned long long _API_CALL fpga_send(unsigned int boardn, unsigned int chnl, HANDLE dma, unsigned long long len32, unsigned long long offset = 0,
    unsigned int last = 1, unsigned int mm_addr = 0, unsigned int mm_addr_inc = 0, unsigned int timeout = DMA_WAIT_FOR_EVER);
...
//return the transfored data length in DWORDs.
//return DMA_START_ERROR for dma start error. use fpga_err_msg() to the get detial error information.
DLLEXPORT unsigned long long _API_CALL fpga_recv(unsigned int boardn, unsigned int chnl, HANDLE dma, unsigned long long len32, 
    unsigned long long offset32 = 0, unsigned int last = 1, unsigned int mm_addr = 0, unsigned int mm_addr_inc = 0, 
    unsigned int timeout = DMA_WAIT_FOR_EVER);

//make a single recv dma for a dma objects array. this is for windows only.
DLLEXPORT unsigned long long _API_CALL fpga_recv_multiple(unsigned int boardn, unsigned int chnl, HANDLE* dma_array, unsigned int dma_num,
    unsigned long long len, unsigned long long offset, unsigned int last, unsigned int mm_addr, unsigned int mm_addr_inc, unsigned int timeout);

//recv dma with infinite mode, no interrupt, no timeout, no wait. return immediately. use fpga_break_dma() to exit.
DLLEXPORT unsigned long long _API_CALL fpga_recv_infinite(unsigned int boardn, unsigned int chnl, HANDLE dma, unsigned long long len32, 
    unsigned long long offset32 = 0, unsigned int last = 1, unsigned int mm_addr = 0, unsigned int mm_addr_inc = 0);
...
DLLEXPORT unsigned int _API_CALL fpga_set_irq_callback(unsigned int boardn, unsigned int user_irq_num, int (*func)(void*), void* param);
...
//20240906 new function
//setup the ring with data buffer unit length and control address parameters
unsigned int _API_CALL fpga_ring_setup(HANDLE _dma, unsigned long long buffer_unit_len32, unsigned int fpga_ring_ctrl_addr);

//Pause the ring
#define fpga_ring_pause(ring) fpga_pause_dma(ring)

//Resume/start the ring dma
DLLEXPORT void _API_CALL fpga_ring_resume(HANDLE ring);
#define fpga_ring_start(ring) fpga_ring_resume(ring)
...
DLLEXPORT void _API_CALL fpga_debug_ring(HANDLE _dma, char* info);
